...
TEST(ohlc, ctor) {
    ...
    EXPECT_EQ(0, d._low);
    EXPECT_EQ(0, d._high);
    EXPECT_EQ(0, d._close);
    EXPECT_EQ(0, d._volume);
    EXPECT_FALSE(d._date.ok());
}

TEST(stock_data, ctor) {
    std::map<date::year_month_day, efl::ohlcv_t> m;
    efl::stock_data_t sd{"SGO", m};

    EXPECT_EQ("SGO", sd.get_stock_id());
    EXPECT_EQ(0, sd.get_data().size());

    efl::ohlcv_t d = {45.12, 45.01, 45.99, 45.87, 256156, 
...
TEST(stock_data, varitation) {
    ...
    efl::ohlcv_t ohlc4 = {90.24, 90.01, 92.99, 91.74, 556156, d4 };
    sd.add_ohlc(ohlc4);
    ...
    efl::ohlcv_t ohlc5 = {70.24, 70.01, 72.99, 71.74, 556156, d5 };
    sd.add_ohlc(ohlc5);
